...
use crate::env::init_file::{get_home_directory, HOME_NAME, MYSQL_CONFIG_JSON, read_file_content, write_string_to_file};
...
impl MySQLStorageManager {
    pub fn new() -> Self {
        MySQLStorageManager { values: read_mysql_json() }
    }
}
...
pub fn write_mysql_json(data: String) {
    if let Ok(home_directory) = get_home_directory().ok_or("无法获取 HOME 目录") {
        write_string_to_file(format!("{}/{}/{}", home_directory, HOME_NAME, MYSQL_CONFIG_JSON).as_str(), data.as_str()).expect("写入mysql-json失败");
    }
}
