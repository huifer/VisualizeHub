...
use crate::env::init_file::{get_home_directory, HOME_NAME, REDIS_CONFIG_JSON, read_file_content, write_string_to_file};
...
impl RedisStorageManager {
    pub fn new() -> Self {
        RedisStorageManager { values: read_redis_json() }
    }
}
...
pub fn write_redis_json(data: String) {
    if let Ok(home_directory) = get_home_directory().ok_or("无法获取 HOME 目录") {
        write_string_to_file(format!("{}/{}/{}", home_directory, HOME_NAME, REDIS_CONFIG_JSON).as_str(), data.as_str()).expect("写入Redis-json失败");
    }
}
