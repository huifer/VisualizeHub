...
use crate::config::mysql_config_storage::{MySQLStorageEntity, MySQLStorageManager, MysqlStorageService};
...
pub async fn get_db_names(id: &str) -> Result<Response<Vec<String>>, ()> {
    ...
    let b = match option {
        ...
        Some(&ref item) => {
            ...
            let operation = crate::op::mysql_op::MysqlOperation::new(&param).await.unwrap();
...
pub async fn get_tables_names(param: GetMysqlTablesParam) -> Result<Response<Vec<String>>, ()> {
    ...
    let b = match option {
        ...
        Some(&ref item) => {
            ...
            let operation = crate::op::mysql_op::MysqlOperation::new(&mup).await.unwrap();
            let response = operation.get_table_names_for_database(param.db_name.as_str()).await;
            Ok(response)
        }
    };
    b
...
#[tauri::command]
pub async fn show_table_create_sql(param: GetMysqlTablesCreatedParam) -> Result<Response<String>, ()> {
    let manager = MySQLStorageManager::new();
    let option = manager.by_id(param.id.as_str());
    ...
    let b = match option {
        ...
        Some(&ref item) => {
            ...
            let operation = crate::op::mysql_op::MysqlOperation::new(&mup).await.unwrap();
            let response = operation.get_table_create_statement(
                param.db_name.as_str(),
                param.table_name.as_str()
            ).await;
            Ok(response)
        }
    };
